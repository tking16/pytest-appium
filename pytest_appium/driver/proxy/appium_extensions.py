        ...
        **kwargs,
        ...
        try:
            return WebDriverWait(
                self, timeout or self.WAIT_FOR_DEFAULT_TIMEOUT_SECONDS
            ).until(getattr(EC, expected_condition)(*args, **kwargs))
...
class AppiumElementHelperMixin:
    ...
    def find_element_safe(self, *args, **kwargs):
        assert (
            len(args) == 2
        ), "find_element should be passed a tuple of (TYPE, LOCATOR)"
        try:
    ...
    def swipe_element(
        ...
        if isinstance(direction, str):
            ...
            assert hasattr(
                Direction, direction
            ), f"Unknown direction {direction}. Should be one of {Direction.__members__.keys()}"
        ...
        swipe_from = center_point_of_moving_axis - (
            range_of_moving_axis * (swipe_distance / 2)
        )
        swipe_to = center_point_of_moving_axis + (
            range_of_moving_axis * (swipe_distance / 2)
        )
        ...
        swipe_params = self.SwipeParams(
            **{
                f"start_{direction.axis.value}": int(
                    direction.signum.value(swipe_from, swipe_to)
                ),
                f"end_{direction.axis.value}": int(
                    direction.signum.inverse.value(swipe_from, swipe_to)
                ),
                f"start_{direction.axis.inverse.value}": int(
                    center_point_of_constant_axis
                ),
                f"end_{direction.axis.inverse.value}": int(
                    center_point_of_constant_axis
                ),
            }
        )
        ...
        assert all(
            map(lambda x: x >= 0, swipe_params)
        ), f"All swipe co-ordinates should be positive {swipe_params}"
        for i in range(0, repeat):
            self.swipe(
                *swipe_params, duration=duration_ms or self._default_swipe_duration
            )
            # sleep(float(sleep_secs))

    def find_element_on_page(
        self, locator, swipe_direction=Direction.UP, max_swipes=4, **kwargs
    ):
        """
